...
MCP (Model Context Protocol) lets Intelli call tools exposed by an MCP server:
- Local subprocess over stdio.
- Remote over HTTP/WebSocket.
...
That extra `[mcp]` flag pulls in the small MCP SDK and its helpers.
